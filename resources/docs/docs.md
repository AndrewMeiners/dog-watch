...
The `home-routes` handler in the `dog-watch.routes.home` namespace
...
The `home-page` function will in turn call the `dog-watch.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `dog-watch.handler` namespace:
...
Request middleware functions are located under the `dog-watch.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `dog-watch.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `dog-watch.db.core` in some other namespace.
